...
export default ({ tableData, reloadData }) => {
  ...
  return ( <div className={'awesomemotive-container'}>
    <div className={'awesomemotive-graph-reload'}>
      <div onClick={ reloadData }><Icon icon={rotateRight}/></div>
    </div>
    <div className={'awesomemotive-graph-container'}>
      { table && Object.keys(graph).length > 0 &&
      ( <table className="graph">
        {table.title &&
        (<caption>{__(table.title, defaultOptions.text_domain)}</caption>)}
        <thead>
          <tr>
        ...
        <tbody>
        {
          Object.keys(graph).map((value, index) => {
            let percentage = (graph[value].value / maxPageViews) * 100
            percentage = Math.round(percentage)

            return (
              <tr key={value} style={{ height: percentage + '%' }}>
                <th scope="row">{ covertData( graph[value].date ) }</th>
                <td><span>{ graph[value].value }</span></td>
              </tr>
            )
          })
        }
