...
describe( 'Table Component', () => {
  ...
  it( 'Should Render the component with ordered list of emails', async () => {
    ...
    expect( container.querySelector('ul > li').innerHTML ).toEqual(props.settings.data.emails[0])
    expect( container.querySelectorAll('ul li').length ).toEqual( props.settings.data.emails.length )
  ...
  it( 'Should Render the component with table containing data', async () => {
    ...
    expect( container.querySelector('table > thead > tr > td').innerHTML ).toEqual('ID')
    expect( container.querySelectorAll('table >  thead > tr td').length ).toEqual( props.tableData.table.data.headers.length )
